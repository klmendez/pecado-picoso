      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          <div className="flex-1 space-y-3 text-center lg:text-left">
            <div className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.32em] text-white/45">
              {current.badge ? <span className="rounded-full border border-white/15 px-3 py-1 text-[10px] tracking-[0.25em] text-white/65">{current.badge}</span> : null}
              <span>{current.category === "gomitas" ? "Gomitas" : "FrutaFresh"}</span>
            </div>
            <h3 className="text-xl sm:text-3xl font-black leading-snug text-white">{current.name}</h3>
            <p className="text-[0.9rem] sm:text-sm text-white/70 leading-relaxed max-w-xl mx-auto lg:mx-0">
              {current.description}
            </p>
            <div className="text-base sm:text-xl font-black text-white/90">{current.priceLabel}</div>
            {current.highlightLine ? (
              <div className="text-[10px] uppercase tracking-[0.2em] text-red-200/80">{current.highlightLine}</div>
            ) : null}
            {current.extraInfo ? (
              <div className="text-[11px] text-white/50">{current.extraInfo}</div>
            ) : null}
          </div>

          <div className="flex-1 flex items-center justify-center">
            <div className="relative h-48 w-48 sm:h-68 sm:w-68 md:h-80 md:w-80 overflow-hidden rounded-[2.5rem] border border-white/15 bg-black/40 shadow-[0_30px_55px_rgba(0,0,0,0.45)]">
              {current.image ? (
                <img
                  src={current.image}
                  alt={current.name}
      ...
      <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            return (
              <button
                ...
                className={[
                  ...
                  active ? "bg-white" : "bg-white/25 hover:bg-white/40",
                ].join(" ")}
        ...
        <div className="flex items-center justify-center gap-3">
          <button
            type="button"
            onClick={handlePrev}
            className="rounded-full border border-white/20 px-3.5 py-1.5 text-[11px] font-black uppercase tracking-[0.2em] text-white/70 transition hover:border-white/40 hover:text-white"
          >
            Anterior
          </button>
          <button
